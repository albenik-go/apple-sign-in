...
func (r *ValidateTokenRequest) Encode() []byte {
	// конечно я тут могу накрутить всякой красоты с тегами и рефлексией, но зачем?
	b := bytes.NewBuffer(nil)
	writeReqParam(b, "client_id", r.ClientId)
	writeReqParam(b, "client_secret", r.ClientSecret)
	writeReqParam(b, "code", r.Code)
	writeReqParam(b, "grant_type", r.GrantType)
	if r.RedirectURI != "" {
		writeReqParam(b, "redirect_uri", r.RedirectURI)
	}
	return b.Bytes()
}
...
func (r *RefreshTokenRequest) Encode() []byte {
	...
	writeReqParam(b, "client_secret", r.ClientSecret)
	writeReqParam(b, "grant_type", r.GrantType)
	writeReqParam(b, "refresh_token", r.RefreshToken)
	return b.Bytes()
}

func writeReqParam(b *bytes.Buffer, k, v string) {
	if b.Len() > 0 {
		b.WriteByte('&')
	}
